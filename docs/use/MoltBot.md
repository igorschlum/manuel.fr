...
Moltbot est conçu pour être plus qu’un simple générateur de texte : c’est un **agent autonome** qui peut, selon les permissions que vous lui donnez :
...
Moltbot est conçu pour être auto-hébergé :
...
Il n’existe pas encore d’interface universelle « officielle » pour guider la configuration complète ; l’installation et les réglages requièrent généralement un peu de confort avec le terminal et la ligne de commande.
...
- **risque d’injection de prompt ou d’exécution involontaire de commandes** si Moltbot est mal configuré ;
- stockage de données sensibles en clair si vous n’êtes pas rigoureux dans vos réglages.

👉 Il est donc impératif de comprendre ce que vous autorisez et comment sécuriser votre instance si vous choisissez d’utiliser Moltbot au quotidien.

---

## 💡 Scénarios d’usage

Voici quelques exemples de ce que Moltbot peut faire une fois installé :
...
Moltbot est un **assistant IA auto-hébergé puissant et modulaire**, qui transforme un simple prompt en une action concrète sur votre système ou vos applications. Contrairement aux agents AI classiques qui attendent vos messages, il peut **prendre des initiatives, se souvenir du contexte et agir en continu**.
...
- [Doc officielle](https://docs.molt.bot/providers/ollama)
